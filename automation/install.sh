...
# get() strips spaces, so read AGENT_CMD raw and take its first word.
AGENT_BIN="$(grep -E '^AGENT_CMD=' "$HERE/config.env" | tail -1 | cut -d= -f2- | awk '{print $1}')"
: "${AGENT_BIN:?AGENT_CMD missing from config.env}"
command -v "$AGENT_BIN" >/dev/null || fail "$AGENT_BIN (from AGENT_CMD) is not on PATH"
command -v python3 >/dev/null || fail "python3 is not on PATH"
say "agent   $AGENT_BIN $("$AGENT_BIN" --version 2>/dev/null | head -1)"
